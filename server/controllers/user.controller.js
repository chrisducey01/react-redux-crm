...
router.route('/')
    .post(authenticateUser, (req, res) => {
        ...
        db.User.create({ username, password, firstName, lastName, role })
            .then(userRes => {
    ...
    .put(authenticateUser, (req, res) => {
        ...
        db.User.findById(id).then(userRes=>{
            ...
            if((req.user.local.username != userRes.local.username) && (req.user.role != 'admin')) 
                return res.status(401).json({message: "You don't have sufficient privileges to change the password for another user.",
